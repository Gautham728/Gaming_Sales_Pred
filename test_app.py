...
if pages == 'Recommendation':
    ...
    st.image('spidy.jpg', width = 500,height =500)
    st.markdown("""<div style="text-align: justify;"> \
...
if pages == 'Regression':
    ...
    if b:
        if model == 'Decision Tree Regressor':
            ...
            # st.write(f"The model estimates your global sales to {np.round(reg_pred, 2)} millions")
            st.write(f"The model estimates your global sales to {np.round(reg_pred[0], 2)} millions")
